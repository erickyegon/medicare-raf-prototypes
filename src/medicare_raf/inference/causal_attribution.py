...
def difference_in_differences(panel, outcome="total_cost", covariates=None):
    ...
    try:
        model = smf.ols(formula, data=df).fit(
            cov_type="cluster", cov_kwds={"groups": df["bene_id"]}
        )
    except Exception:
        model = smf.ols(formula, data=df).fit()
    ...
    return {
        ...
        "att_se": round(float(se), 2),
        "ci_95_low": round(float(ci[0]), 2),
        "ci_95_high": round(float(ci[1]), 2),
        "p_value": round(float(p_val), 4),
        "significant": bool(p_val < 0.05),
        "n_treated": int((df["treat"] == 1).sum() // 2),
        "n_control": int((df["treat"] == 0).sum() // 2),
...
def propensity_score_matching(panel, outcome="total_cost", caliper=0.05):
    ...
    pre["risk_high"] = (
        (pre["risk_tier"] == "high").astype(int) if "risk_tier" in pre.columns else 0
    )
    pre["risk_moderate"] = (
        (pre["risk_tier"] == "moderate").astype(int)
        if "risk_tier" in pre.columns
        else 0
    )
    ps_feats = [
        f
    ...
    lr = LogisticRegression(max_iter=500, random_state=42).fit(
    ...
    print(
        f"   [PSM] Matching {len(treated):,} treated to {len(control):,} controls (vectorized)..."
    )
    ...
    if n_pairs < 50:
        return {
            "method": "PSM",
            "outcome": outcome,
            "error": f"Only {n_pairs} matches. Try larger caliper.",
        }
    ...
    return {
        "method": "PSM (1:1 NN vectorized)",
        "outcome": outcome,
        "att": round(att, 2),
        "att_se": round(se, 2),
        "ci_95_low": round(att - 1.96 * se, 2),
        "ci_95_high": round(att + 1.96 * se, 2),
...
def generate_love_plot(
    ...
    for cov in covariates:
        if cov in treated_pre.columns and cov in control_pre.columns:
            ...
            smd_after = standardized_mean_difference(
                treated_matched, control_matched, cov
            )
    ...
    ax.axvline(
        0.1, color="red", linestyle="--", linewidth=1, label="SMD = 0.1 (good balance)"
    )
    ax.axvline(
        0.2, color="orange", linestyle="--", linewidth=1, label="SMD = 0.2 (acceptable)"
    )
    ...
    ax.set_title(
        "Covariate Balance: Love Plot (Before vs After Propensity Score Matching)"
    )
    ax.legend()
...
def project_shared_savings(
    att_pmpm,
    n_attributed_lives,
    benchmark_pmpm=9800.0,
...
def run_full_attribution(panel, n_attributed_lives=None, benchmark_pmpm=9800.0):
    ...
    print(
        f"   ATT (total cost): ${did_cost['att']:,.2f}/member (p={did_cost['p_value']:.4f})"
    )
    ...
    if "att" in psm_cost:
        print(
            f"   ATT (PSM): ${psm_cost['att']:,.2f}/member (p={psm_cost['p_value']:.4f})"
        )
        print(f"   Matched pairs: {psm_cost['n_matched_pairs']:,}")
        print(
            f"   Post-match SMD (age): {psm_cost['smd_age_post_match']:.3f} (target <0.10)"
        )
        ...
        if all(cov in psm_cost["treated_pre"].columns for cov in covariates):
            generate_love_plot(
                psm_cost["treated_pre"],
                psm_cost["control_pre"],
                psm_cost["treated_matched"],
                psm_cost["control_matched"],
                covariates,
                save_path="reports/figures/05_love_plot.png",
            )
            print("   → Love Plot saved: reports/figures/05_love_plot.png")
    else:
    ...
    savings = project_shared_savings(
        did_cost["att"], n_attributed_lives, benchmark_pmpm
    )
